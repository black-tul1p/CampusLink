...
function ViewPastQuiz(props) {
  ...
  const UpdateAttempt = async () => {
    // Calculate and set sum of alloted points
    let pointSum = 0;
    for (let i = 0; i < studentAnswers.length; i++) {
      const answer = studentAnswers[i];

      if (answer.points && !isNaN(answer.points)) {
        pointSum += parseInt(answer.points);
      }
    }
      ...
      >
        ...
        <div className="main-quiz-box">
          {isGraded ? (
            ...
            >
              <p style={{ color: "green" }}>
                {"Score:  " + studentPoints + "/" + quizPoints + " pts"}
              </p>
              {isEdit === false ? (
                <Button
          ...
          {attemptedOn !== null && (
            <label style={{ color: "black" }}>
              {"Attempted On: " + attemptedOn.toDate()}
            </label>
          )}
          ...
          {quizQuestions.map((question, index) => (
            <>
              <div className="question-header">
                ...
                studentAnswers[index].points !== undefined ? (
                  <>
                    {isEdit ? (
                      <div
                        style={{
                      ...
                      <label style={{ color: "black" }}>
                        {studentAnswers[index].points} / {question.points}
                      </label>
                    )}
                  </>
                ) : (
